...
- (void)initUI {
    ...
    self.titleLabel.textColor = RGB_HEX(0xffffff, 1);
    // 添加字符串选择器
...
- (void)loadData {
    ...
    if (self.isSingleColumn) {
        ...
        if (self.selectedItems == nil ||
            self.selectedItems.count != self.dataSource.count ||
...
- (UIPickerView *)pickerView {
    if (!_pickerView) {
        ...
        _pickerView.backgroundColor = [UIColor colorWithRed:17/255.0 green:44/255.0 blue:58/255.0 alpha:1];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    UILabel *lblcontent = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 30)];
    lblcontent.textColor = RGB_HEX(0xffffff, 1);
    lblcontent.textAlignment = NSTextAlignmentCenter;
    for (int i = 0; i<self.dataSource.count; i++) {
        if (row == i) {
            lblcontent.text = self.dataSource[i];
        }
    }
    return lblcontent;
