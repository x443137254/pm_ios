...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    {
        //左上角文字
        _lblTitleY = [[UILabel alloc] initWithFrame:CGRectMake(15, 10, 200, 20)];
        _lblTitleY.font = [UIFont systemFontOfSize:14];
        _lblTitleY.textAlignment = NSTextAlignmentLeft;
        _lblTitleY.backgroundColor = [UIColor clearColor];
        
        //下面文字
        _lblTitleX = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 200, 20)];
        _lblTitleX.center = CGPointMake(self.bounds.size.width / 2, self.bounds.size.height - 10 - 20 / 2);
        _lblTitleX.font = [UIFont systemFontOfSize:14];
        _lblTitleX.textAlignment = NSTextAlignmentCenter;
        _lblTitleX.backgroundColor = [UIColor clearColor];
        
        
        //中间区域
        _contentView = [[UIView alloc]initWithFrame:CGRectMake(0, 10 + _lblTitleY.frame.size.height + 10, self.bounds.size.width, self.bounds.size.height - (10 + _lblTitleY.frame.size.height + 10) - 40)];
...
- (void)startDraw{
    ...
    _lblTitleY.textColor = self.colorTitleY;
    _lblTitleX.textColor = self.colorTitleX;
    
    [self addSubview:_lblTitleY];
    [self addSubview:_lblTitleX];
...
- (void)addLineChartView
{
    ...
    //边框
    _lineChartView.layer.borderColor = [UIColor colorWithRed:216/255.0 green:216/255.0  blue:216/255.0  alpha:0].CGColor;
...
-(void)addYAxisViews
{
    ...
    for (int i = 0;i< _dataArrOfY.count ;i++ )
    {
        ...
        leftLabel.textColor = self.colorTextY;
...
-(void)addXAxisViews
{
    ...
    for (int i = 0;i< _dataArrOfX.count;i++ )
    {
        ...
        leftLabel.font = [UIFont systemFontOfSize:10];
        leftLabel.textColor = self.colorTextX;
...
-(void)addLinesView
{
    ...
    for (int i = 0;i < _dataArrOfY.count - 2 ;i++ )
    {
        ...
        hengView.backgroundColor = self.colorLineX;
    ...
    for (int i = 0;i< _dataArrOfX.count - 2 ;i++ )
    {
        ...
        shuView.backgroundColor = self.colorLineY;
...
-(void)addBezierLine
{
    ...
    shapeLayer.strokeColor = self.colorLine.CGColor;
    ...
    shadeLayer.fillColor = self.colorGradientTop.CGColor;
    ...
    gradientLayer.colors = @[(__bridge id)self.colorGradientCenter.CGColor,(__bridge id)self.colorGradientButtom.CGColor];
