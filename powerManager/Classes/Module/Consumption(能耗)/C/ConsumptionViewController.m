...
@interface ConsumptionViewController () <ConsumptionViewControllerDelegate> {
    ConsumptionView *consumptionView;
    AirConditionViewController *ACVC;
    AddDeviceViewController *ADVC;
    
    ConsumptionViewPad *CVP;
...
- (void)setConsumptionViewPhone{
    UIImageView *backImageView=[[UIImageView alloc]initWithFrame:self.view.frame];
    [backImageView setImage:[UIImage imageNamed:@"bg"]];
    [self.view addSubview:backImageView];
    if (!consumptionView) {
        consumptionView = [[ConsumptionView alloc] initWithFrame:CGRectMake(0, 64, self.view.frame.size.width, self.view.frame.size.height-64-44*ProportionX)];
    }
    consumptionView.customDelegate = self;
    [self.view addSubview:consumptionView];
}
...
- (void)loadFootViewPhone{
    ...
    [btnConsumption setUserInteractionEnabled:NO];
    [btnConsumption setAdjustsImageWhenHighlighted:NO];
    [btnConsumption setTitleColor:[UIColor colorWithRed:0 green:255/255.0 blue:240/255.0 alpha:1] forState:UIControlStateNormal];
    [btnConsumption setTitle:@"能耗" forState:UIControlStateNormal];
    btnConsumption.titleLabel.font = [UIFont systemFontOfSize:12];
    [btnConsumption setImage:[UIImage imageNamed:@"能耗_click"] forState:UIControlStateNormal];
    ...
    UIImageView *img = [[UIImageView alloc] initWithFrame:CGRectMake(btnConsumption.frame.origin.x, btnConsumption.frame.origin.y + btnConsumption.frame.size.height/2.0, btnConsumption.frame.size.width, btnConsumption.frame.size.height/2.0)];
...
- (void)setCVP{
    if (!CVP) {
        CVP = [[ConsumptionViewPad alloc] initWithFrame:CGRectMake(69 + 15, 30, SCREEN_WIDTH - 69 - 30, SCREEN_HEIGHT - 60)];
        CVP.backgroundColor = [UIColor clearColor];
    }
    [self.view addSubview:CVP];
}
...
- (void)loadLeftViewPad{
    if (!leftView) {
        ...
        [btnConsumption setTitleColor:[UIColor colorWithRed:0/255.0 green:255/255.0 blue:264/255.0 alpha:1] forState:UIControlStateNormal];
        [btnConsumption setTitle:@"能耗" forState:UIControlStateNormal];
        ...
        UIImageView *btnBG = [[UIImageView alloc] initWithFrame:CGRectMake(0, 154+69*2, width, width)];
        ...
        [btnAnalysis setTitleColor:[UIColor colorWithRed:0/255.0 green:111/255.0 blue:164/255.0 alpha:1] forState:UIControlStateNormal];
        [btnAnalysis setTitle:@"分析" forState:UIControlStateNormal];
...
- (void)btnClickWithTag:(NSString *)tag{
    NSLog(@"tag = %@",tag);
    if ([tag isEqualToString:@"AirCondition"]) {
        if (!ACVC) {
            ACVC = [[AirConditionViewController alloc] init];
        }
        [self.navigationController pushViewController:ACVC animated:YES];
    } else if ([tag isEqualToString:@"AddDevice"]) {
        if (!ADVC) {
            ADVC = [[AddDeviceViewController alloc] init];
        }
